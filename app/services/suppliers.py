...
def list_supplier_candidates(connection: Connection) -> list[dict[str, Any]]:
    rows = connection.execute(
        """
        SELECT factory, COUNT(*) AS occurrence_count
        FROM quotation_items
        WHERE supplier_id IS NULL
          AND TRIM(COALESCE(factory, '')) != ''
        GROUP BY factory
        ORDER BY occurrence_count DESC, factory
        """
    ).fetchall()
    candidates = []
    for row in rows:
        match = match_supplier_by_name(connection, row["factory"])
        candidates.append(
            {
                "factory": row["factory"],
...
def link_supplier_candidate(
    ...
    *,
    ...
    add_alias_text_alias(connection, supplier_id, factory_name)
    sync_supplier_aliases(connection, supplier_id, operator_name)
    updated_count = update_quotation_items_for_factory(
        connection,
    ...
    create_operation_log(
        ...
        note=f"工厂={factory_name}; 供应商ID={supplier_id}",
    )
    return updated_count


def create_supplier_from_candidate(
    connection: Connection,
    *,
    factory_name: str,
    operator_name: str,
    ...
    create_operation_log(
        connection,
        operator_name=operator_name,
        action_type="supplier_candidate_created",
        row_count=updated_count,
        note=f"工厂={factory_name}; 供应商ID={supplier_id}",
    )
    return supplier_id
...
def update_quotation_items_for_factory(
    connection: Connection,
    *,
    factory_name: str,
    supplier_id: int,
